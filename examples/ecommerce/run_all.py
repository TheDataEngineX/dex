...
def demo_data() -> tuple[list[dict], list[dict], list[dict]]:
    ...
    _ok(f"Bronze  : {len(raw_customers)} customers, {len(raw_orders)} orders, "
    ...
    for r in raw_customers:
        if int(r["age"]) > 0 and int(r["tenure_days"]) >= 0:
            if r["customer_id"] not in seen_ids:
                seen_ids.add(r["customer_id"])
                silver_customers.append(dict(r))

    # Silver: filter orders
    silver_orders = [
        dict(r) for r in raw_orders
    ...
    result = gate.evaluate(
        silver_customers, DataLayer.SILVER, dataset_name="silver_customers"
    )
    status = "PASS" if result.passed else "FAIL"
    _ok(f"Silver quality gate [{status}] score={result.quality_score:.3f} "
    ...
    for order in silver_orders:
        ...
        if cid not in revenue:
            revenue[cid] = {
                "customer_id": cid, "order_count": 0,
                "lifetime_value": 0.0, "refund_count": 0,
            }
    ...
    _ok(f"Gold quality gate [{'PASS' if gold_result.passed else 'FAIL'}] "
        f"score={gold_result.quality_score:.3f} n={gold_result.record_count}")

    layer_summary = MedallionArchitecture.get_all_layers()
    _ok(f"Medallion layers: {[l.layer_name for l in layer_summary]}")
...
def demo_rag() -> None:
    ...
    docs = [
        f"{p['name']} (${p['price']}, rating {p['rating']}): {p['description']}"
        for p in products
    ]
    docs += [
...
def demo_lineage(
    ...
    bronze_cust = lineage.record(
        ...
        pipeline_name="clean_customers", step_name="csv_ingest",
        quality_score=1.0,
    )
    lineage.record(
        ...
        input_count=20, output_count=len(silver_customers),
        pipeline_name="clean_customers", step_name="filter+dedup",
        quality_score=0.97, parent_id=bronze_cust.event_id,
    )
    bronze_ord = lineage.record(
        operation="ingest", layer="bronze",
        source="s3://shopmetrics/raw/orders.csv",
        destination="bronze.raw_orders",
        input_count=50, output_count=50,
        pipeline_name="clean_orders", step_name="csv_ingest",
        quality_score=1.0,
    )
    silver_ord_evt = lineage.record(
        ...
        input_count=50, output_count=len(silver_orders),
        pipeline_name="clean_orders", step_name="filter",
        quality_score=0.99, parent_id=bronze_ord.event_id,
    )
    gold_evt = lineage.record(
        operation="enrich", layer="gold",
        source="silver.silver_orders",
        destination="gold.customer_revenue",
        input_count=len(silver_orders), output_count=len(gold_revenue),
        pipeline_name="customer_revenue", step_name="sql_agg",
        quality_score=1.0, parent_id=silver_ord_evt.event_id,
    ...
    for evt in chain:
        _info(f"  [{evt.layer:6s}] {evt.source} → {evt.destination} "
